...
function loadedHandler(){
	...
	CKobject.ajax('post','utf-8','ckplayer/barrage.php?path='+bgPath,function(data){
		if(data){
			var obj=eval('(' + data + ')');
			bgArr=obj['ckplayer'];
		}
...
function barrage(s){//写弹幕，写完同时把所有的读取出来
	...
	if(!nt)nt=0;
	if(nt<0)nt=0;
	CKobject.ajax('post','utf-8','ckplayer/barrage.php?path='+bgPath+'&s='+encodeURIComponent(s)+'&j='+nt,function(data){
		if(data){
			var obj=eval('(' + data + ')');
			bgArr=obj['ckplayer'];
		}
